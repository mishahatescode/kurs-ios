...
private struct KeypadButton: View {
  let key: String
  let isDisabled: Bool
  /// Fires on a long-press (currently only wired up for "⌫") — clears the
  /// whole amount instead of deleting one digit at a time.
  ...
  var body: some View {
    ...
    .simultaneousGesture(
      LongPressGesture(minimumDuration: 0.5).onEnded { _ in
        guard !isDisabled, let onLongPress else { return }
