...
final class PersistenceService {
    ...
    // MARK: - Keys
    private enum Key {
        static let pinnedCurrencies  = "kurs.pinned"
    ...
    func saveRates(ecb: [String: Double], live: [String: Double], date: Date) {
        if let ecbData  = try? JSONEncoder().encode(ecb)  { defaults.set(ecbData,  forKey: Key.cachedECBRates) }
        if let liveData = try? JSONEncoder().encode(live) { defaults.set(liveData, forKey: Key.cachedLiveRates) }
